...
init_process_group('nccl', world_size=1, rank=0)
...
if Path("models/reresnet34-generator.pt").exists():
    ...
    generator = torch.load('models/reresnet34-generator.pt').cpu()
else:
...
if Path('models/reresnet34-discriminator.pt').exists():
    ...
    discriminator = torch.load('models/reresnet34-discriminator.pt').cpu()
else:
