...
if Path("models/resnet34-generator.pt").exists():
    print("Loading saved generator...")
    generator = torch.load('models/resnet34-generator.pt')
else:
    print("Initializing new generator...")
    generator = ResUNet34().cuda()

if Path('models/resnet34-discriminator.pt').exists():
    print("Loading saved discriminator...")
    discriminator = torch.load('models/resnet34-discriminator.pt')
else:
    print("Initializing new discriminator...")
    discriminator = Resnet34Discriminator().cuda()

if Path('models/resnet34-SeGAN.json').exists():
    print("Found training state dict, will resume at correct iteration.")
    with Path('models/resnet34-SeGAN.json').open('r') as f:
...
for epoch in tqdm(range(EPOCHS), desc="Training"):
    ...
    for img, mask in tqdm(loader, total=len(dataset)//BATCH_SIZE, desc="Epoch", leave=False):
        ...
        if i % REDRAW_INTERVAL == 1:
            ...
            info_dict['iter'] = i+1
            with Path('models/resnet34-SeGAN.json').open('w+') as f:
                json.dump(info_dict, f)

            torch.save(generator, f"models/resnet34-generator.pt.tmp")
            torch.save(discriminator, f"models/resnet34-discriminator.pt.tmp")
            rename("models/resnet34-generator.pt.tmp", "models/resnet34-generator.pt")
            rename("models/resnet34-discriminator.pt.tmp", "models/resnet34-discriminator.pt")
        i += 1
        even = not even
